#!/usr/bin/env python3
# ===================================================================
# Maze3_eval.py - Evaluation for Maze3 Models
# 
# DESCRIPTION:
# This script provides comprehensive evaluation for Maze3 models
# with full visualization capabilities.
# 
# FEATURES:
# - Model structure analysis
# - Support for "maze3", "maze3_fmsl"
...
# Import models
try:
    from maze3 import Model_Maze5
    from maze3_fmsl_standardized import Model3_RawNetSinc_SE_Transformer_FMSL_Standardized
    print("✅ Successfully imported Maze3 models")
    ...
    for batch_size, seq_len in test_cases:
        try:
            ...
            if isinstance(output, dict):
                ...
                print(f"  FMSL model output keys: {list(output.keys())}")
            else:
...
def check_model_compatibility(model, model_name, device):
    ...
    try:
        ...
        if isinstance(output, dict):
            # FMSL models return a dictionary with 'logits' key
            output_tensor = output['logits']
            print(f"  FMSL model detected, using 'logits' key")
        else:
            # Regular models return a tensor directly
            output_tensor = output
            ...
            for batch_idx, (batch_x, utt_id) in enumerate(data_loader):
                try:
                    ...
                    if torch.any(torch.isnan(batch_out_tensor)) or torch.any(torch.isinf(batch_out_tensor)):
                        print(f"Warning: NaN/Inf detected in batch {batch_idx}, attempting to fix...")
                        
                        # Try to fix NaN/Inf by replacing with small random values
                        if torch.any(torch.isnan(batch_out_tensor)):
                            batch_out_tensor = torch.where(torch.isnan(batch_out_tensor), 
                                                         torch.randn_like(batch_out_tensor) * 0.01, 
                                                         batch_out_tensor)
                        if torch.any(torch.isinf(batch_out_tensor)):
                            batch_out_tensor = torch.where(torch.isinf(batch_out_tensor), 
                                                         torch.randn_like(batch_out_tensor) * 0.01, 
                                                         batch_out_tensor)
                        
                        # Check if fix worked
                        if torch.any(torch.isnan(batch_out_tensor)) or torch.any(torch.isinf(batch_out_tensor)):
                            print(f"  Fix failed, skipping batch {batch_idx}")
                            error_count += 1
                            ...
                            print(f"  Fix successful for batch {batch_idx}")
...
def create_evaluation_plots(metrics, model_name, output_dir):
    ...
    if not metrics:
        print("❌ No valid metrics data for plotting")
        return
    
    # For plotting, we need to recalculate y_true and y_scores from the scores file
    # This is a simplified version that just shows the metrics without detailed plots
    print("📊 Evaluation metrics summary:")
    print(f"   Total samples: {metrics.get('total_samples', 'N/A')}")
    print(f"   Bonafide samples: {metrics.get('bonafide_samples', 'N/A')}")
    print(f"   Spoof samples: {metrics.get('spoof_samples', 'N/A')}")
    print(f"   EER: {metrics.get('eer_percentage', 'N/A'):.2f}%")
    print(f"   MinDCF: {metrics.get('min_dcf', 'N/A'):.4f}")
    
    print("✅ Metrics summary created (detailed plots require y_true/y_scores data)")
...
def main():
    parser = argparse.ArgumentParser(description='Maze3 Model Evaluation')
    parser.add_argument('--model_type', type=str, required=True, choices=['maze3', 'maze3_fmsl'],
    ...
    parser.add_argument('--batch_size', type=int, default=352,
                       help='Batch size for evaluation (reduced for FMSL models)')
    ...
    print(f"{'='*80}")
    print(f"Maze3 MODEL EVALUATION SYSTEM")
    print(f"{'='*80}")
    ...
    # Model configuration - Different for regular vs FMSL models
    if args.model_type == 'maze3':
        # Regular maze3 uses base wav2vec2 model
        model_config = {
            ...
            # Wav2Vec2 specific parameters - Use base model for regular maze3
            'wav2vec2_model_name': 'facebook/wav2vec2-base-960h',  # Use base model for regular maze3
            'wav2vec2_output_dim': 768,  # Base model output dimension
            'wav2vec2_freeze': True
        }
    else:  # maze3_fmsl
        # FMSL maze3 uses large wav2vec2 model
        model_config = {
            'filts': [128, [128, 128], [128, 256]],
            'first_conv': 251,
            'sample_rate': 16000,
            ...
            'spec_aug_time_mask_param_raw': 10,
            'spec_aug_n_time_masks_raw': 1,
            # Wav2Vec2 specific parameters - Use large model for FMSL maze3
            'wav2vec2_model_name': 'facebook/wav2vec2-large-960h',  # Use large model for FMSL maze3
            'wav2vec2_output_dim': 1024,  # Large model output dimension
            'wav2vec2_freeze': True,
            'wav2vec2_freeze_cnn': True,  # For FMSL models
            # Transformer parameters for FMSL models
            'transformer_hidden_dim': 512,
            'transformer_num_layers': 6,
            'transformer_num_heads': 8,
            'transformer_dropout': 0.1,
            # FMSL parameters for FMSL models
            'fmsl_type': 'prototype',
            'fmsl_n_prototypes': 3,
            'fmsl_s': 15.0,
            'fmsl_m': 0.35
    ...
    try:
        if args.model_type == 'maze3':
            # Import and initialize Maze3 model
            model = Model_Maze5(model_config, device)
            model_name = "Maze3 (RawNetSinc + SE Transformer)"
        elif args.model_type == 'maze3_fmsl':
            # Import and initialize Maze3 FMSL model
            model = Model3_RawNetSinc_SE_Transformer_FMSL_Standardized(model_config, device)
            model_name = "Maze3 FMSL (RawNetSinc + SE Transformer + FMSL)"
    ...
    try:
        ...
        if isinstance(checkpoint, dict):
            if 'model_state_dict' in checkpoint:
                # This is a training checkpoint with model_state_dict key
                print("📦 Detected training checkpoint format, extracting model_state_dict")
        ...
        print("2. For FMSL models, make sure you're using --model_type maze3_fmsl")
        print("3. For regular Maze3 models, make sure you're using --model_type maze3")
    ...
    # Forward pass testing with NaN detection
    forward_results = test_model_forward_pass(model, device, model_name)
    
    # Additional NaN debugging for FMSL models
    if args.model_type == 'maze3_fmsl':
        print(f"\n{'='*60}")
        print("DEBUGGING FMSL MODEL FOR NaN ISSUES")
        print(f"{'='*60}")
        
        # Test with a simple input
        test_input = torch.randn(1, 64600).to(device)
        model.eval()
        ...
        with torch.no_grad():
            try:
                ...
                wav2vec2_features = model.wav2vec2_extractor(test_input)
                print(f"  Wav2Vec2 output shape: {wav2vec2_features.shape}")
                print(f"  Wav2Vec2 output stats: mean={wav2vec2_features.mean().item():.6f}, std={wav2vec2_features.std().item():.6f}")
                print(f"  Wav2Vec2 has NaN: {torch.isnan(wav2vec2_features).any().item()}")
                
                print("Testing feature projection...")
                projected_features = model.feature_projection(wav2vec2_features)
                print(f"  Projected output shape: {projected_features.shape}")
                print(f"  Projected output stats: mean={projected_features.mean().item():.6f}, std={projected_features.std().item():.6f}")
                print(f"  Projected has NaN: {torch.isnan(projected_features).any().item()}")
                
                print("Testing residual blocks...")
                features = projected_features
                for i, block in enumerate(model.blocks):
                    features = block(features)
                    print(f"  Block {i} output stats: mean={features.mean().item():.6f}, std={features.std().item():.6f}")
                    print(f"  Block {i} has NaN: {torch.isnan(features).any().item()}")
                    if torch.isnan(features).any():
                        print(f"  ❌ NaN detected in block {i}!")
                        break
                
                print("Testing final block...")
                features = model.final_block(features)
                print(f"  Final block output stats: mean={features.mean().item():.6f}, std={features.std().item():.6f}")
                print(f"  Final block has NaN: {torch.isnan(features).any().item()}")
                
                print("Testing transformer...")
                features = model.transformer(features)
                print(f"  Transformer output stats: mean={features.mean().item():.6f}, std={features.std().item():.6f}")
                print(f"  Transformer has NaN: {torch.isnan(features).any().item()}")
                
                print("Testing global pooling...")
                features = model.global_pool(features).squeeze(-1)
                print(f"  Global pool output stats: mean={features.mean().item():.6f}, std={features.std().item():.6f}")
                print(f"  Global pool has NaN: {torch.isnan(features).any().item()}")
                
                print("Testing FMSL system...")
                fmsl_output = model.fmsl_system(features, None, training=False)
                print(f"  FMSL output keys: {list(fmsl_output.keys())}")
                if 'logits' in fmsl_output:
                    logits = fmsl_output['logits']
                    print(f"  FMSL logits stats: mean={logits.mean().item():.6f}, std={logits.std().item():.6f}")
                    print(f"  FMSL logits has NaN: {torch.isnan(logits).any().item()}")
    ...
    # For FMSL models, use even smaller batch size
    if args.model_type == 'maze3_fmsl' and args.batch_size > 500:
        print(f"⚠️ FMSL model detected, reducing batch size to 8 for memory efficiency")
        args.batch_size = 8
    ...
    try:
        ...
        # Create plots if metrics are available
        if metrics:
            create_evaluation_plots(metrics, model_name, args.output_dir)
