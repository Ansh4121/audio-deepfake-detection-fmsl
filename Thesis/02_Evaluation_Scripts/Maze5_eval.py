#!/usr/bin/env python3
# ===================================================================
# Maze5_eval_clean.py - Clean Evaluation for MAZE5 and MAZE5 FMSL Models
# 
# DESCRIPTION:
# This script provides comprehensive evaluation for both MAZE5 and MAZE5 FMSL models
# with full visualization capabilities matching MAZE6 evaluation.
# 
# FEATURES:
# - Model structure analysis
# - Support for both MAZE5 and MAZE5 FMSL models
# - Full dataset evaluation (71,237 samples)
...
# Import models
try:
    from maze5 import Model5_RawNetSinc_SpecAugment_FocalLoss
    from maze5_fmsl_standardized import Model5_RawNetSinc_SpecAugment_FocalLoss_FMSL_Standardized
    print("✅ Successfully imported MAZE5 models")
...
def check_model_compatibility(model, model_name, device):
    ...
    try:
        ...
        if len(output.shape) == 2 and output.shape[1] == 2:
            ...
            print(f"❌ Output shape incorrect: {output.shape}, expected [batch_size, 2]")
...
def main():
    parser = argparse.ArgumentParser(description='Simple MAZE5 Model Evaluation')
    parser.add_argument('--model_type', type=str, required=True, choices=['maze5', 'maze5_fmsl'],
    ...
    print(f"{'='*80}")
    print(f"MAZE5 MODEL EVALUATION SYSTEM")
    print(f"{'='*80}")
    ...
    try:
        if args.model_type == 'maze5':
            # Import and initialize MAZE5 model
            from maze5 import Model5_RawNetSinc_SpecAugment_FocalLoss
            model = Model5_RawNetSinc_SpecAugment_FocalLoss(model_config, device)
            model_name = "MAZE5 (RawNetSinc + SpecAugment + Focal Loss)"
        elif args.model_type == 'maze5_fmsl':
            # Import and initialize MAZE5 FMSL model
            from maze5_fmsl_standardized import Model5_RawNetSinc_SpecAugment_FocalLoss_FMSL_Standardized
            model = Model5_RawNetSinc_SpecAugment_FocalLoss_FMSL_Standardized(model_config, device)
            model_name = "MAZE5 FMSL (RawNetSinc + SpecAugment + Focal Loss + FMSL)"
    ...
    try:
        ...
        print("2. For FMSL models, make sure you're using --model_type maze5_fmsl")
        print("3. For regular MAZE5 models, make sure you're using --model_type maze5")
