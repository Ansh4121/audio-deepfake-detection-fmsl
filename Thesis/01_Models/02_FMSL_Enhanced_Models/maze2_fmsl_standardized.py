#!/usr/bin/env python3
# ===================================================================
# maze2_fmsl_standardized.py - Model 2: Wav2Vec2 + SpecAugment + Focal Loss + Standardized FMSL
# 
# DESCRIPTION:
# This is the STANDARDIZED version of maze2 with proper FMSL implementation
...
# - ✅ Wav2Vec2 feature extraction with SpecAugment
...
class Residual_Block_SE(nn.Module):
    def __init__(self, nb_filts_in_out, first=False, dropout_rate=0.3, stride=1, use_se=True, se_reduction=16):
        ...
        # SE block
        if self.use_se:
            self.se = SEBlock(nb_filts_in_out[1], reduction=se_reduction)
...
# ===================================================================
# Part 2: Model 2 - Wav2Vec2 + SpecAugment + Focal Loss + FMSL (Checkpoint Compatible)
# ===================================================================

class Model2_Wav2Vec2_SpecAugment_Focal_FMSL(nn.Module):
    """Model 2: Wav2Vec2 + SpecAugment + Focal Loss + FMSL - Compatible with checkpoint"""
    def __init__(self, d_args, device):
        super(Model2_Wav2Vec2_SpecAugment_Focal_FMSL, self).__init__()
        self.device = device
        ...
        # First batch norm (matches checkpoint)
        self.first_bn = nn.BatchNorm1d(d_args['filts'][0])
        self.selu = nn.SELU(inplace=False)

        # Residual blocks with SE (matches checkpoint structure)
        self.block0 = Residual_Block_SE(d_args['filts'][0], first=True, use_se=True, se_reduction=16)
        self.block1 = Residual_Block_SE(d_args['filts'][1], stride=2, use_se=True, se_reduction=16)
        self.block2 = Residual_Block_SE(d_args['filts'][2], use_se=True, se_reduction=16)  # Use filts[2] for block2

        # Global pooling
        self.avg_pool = nn.AdaptiveAvgPool1d(1)

        # FMSL system (matches checkpoint dimensions)
        fmsl_config = create_fmsl_config(
            model_type=d_args.get('fmsl_type', 'prototype'),
            n_prototypes=d_args.get('fmsl_n_prototypes', 3),
            s=d_args.get('fmsl_s', 32.0),
        ...
        # Use 256 dimensions to match checkpoint (block2 output)
        self.fmsl_system = AdvancedFMSLSystem(
            input_dim=256,  # Match checkpoint dimensions (block2 output)
            n_classes=d_args['nb_classes'],
            use_integrated_loss=False,
            **fmsl_config
        )
    ...
    def forward(self, x, labels=None, training=False):
        ...
        # Project to FMSL input dimension (512)
        if out.size(1) != 512:
            # Add a projection layer if needed
            if not hasattr(self, 'fmsl_projection'):
                self.fmsl_projection = nn.Linear(out.size(1), 512).to(self.device)
            out = self.fmsl_projection(out)
        
        # FMSL system handles geometric features and classification
        fmsl_output = self.fmsl_system(out, labels, training=training)
        
        # Use CCE loss for consistency
        if training and labels is not None:
            loss = self.criterion(fmsl_output['logits'], labels)
            return {
                'logits': fmsl_output['logits'],
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Maze2+Standardized FMSL: Wav2Vec2 + SpecAugment + Focal Loss + True Geometric FMSL')
    ...
    parser.add_argument('--comment', type=str, default="maze2_wav2vec2_fmsl_standardized_asvspoof2021", help='Comment for model directory')
    ...
    # STANDARDIZED Model 2 configuration for fair statistical analysis
    model_config = {
        'filts': [128, [128, 128], [128, 256]],  # Standardized architecture
        'first_conv': 251,
        'sample_rate': 16000,
        'nb_fc_node': 1024,
        'fc_dropout': 0.5,
        ...
        # FMSL parameters - STANDARDIZED for fair comparison
        'fmsl_type': 'prototype',
        'fmsl_n_prototypes': 3,
        'fmsl_s': 32.0,
        'fmsl_m': 0.45,
        'fmsl_enable_lsa': False,
        # Additional parameters for statistical analysis
        'dropout_rate': 0.3,
    ...
    model = Model2_Wav2Vec2_SpecAugment_Focal_FMSL(model_config, device)
    if args.model_path:
    ...
    print(f'✅ MAZE2 STANDARDIZED FMSL implementation completed!')
    print(f'✅ Wav2Vec2 + SpecAugment + Focal Loss + True Geometric FMSL!')
    print(f'✅ Standardized for fair statistical analysis vs other mazes!')
