#!/usr/bin/env python3
# ===================================================================
# maze8.py - Model 6 with FMSL (Frequency-Modulated Spectral Loss)
# 
# DESCRIPTION:
# This model extends Model 6 (RawNet + Wav2Vec2) with the novel FMSL
# layer for improved frequency-domain feature learning. This represents
# your novel contribution to the field.
# 
# STANDARDIZATION:
...
# ===================================================================
# Part 1: Model Definition (Model 6 + FMSL)
# ===================================================================
...
class FMSLLayer(nn.Module):
    """
    Frequency-Modulated Spectral Loss Layer
    This is the novel contribution that enhances frequency-domain feature learning
    """
    def __init__(self, input_dim, num_filters=64, kernel_size=7, dropout=0.1):
        super(FMSLLayer, self).__init__()
        ...
        self.spectral_enhancement = nn.Sequential(
        ...
        self.freq_attention = nn.Sequential(
            ...
            nn.Conv1d(num_filters, num_filters // 4, kernel_size=1),
            nn.ReLU(inplace=True),
            nn.Conv1d(num_filters // 4, num_filters, kernel_size=1),
            nn.Sigmoid()
        )
        
        # Output projection
        self.output_projection = nn.Conv1d(num_filters, input_dim, kernel_size=1)
        
    def forward(self, x):
        # Apply frequency modulation
        modulated = self.freq_modulation(x)
        
        # Apply spectral enhancement
        enhanced = self.spectral_enhancement(modulated)
        
        # Apply frequency attention
        attention_weights = self.freq_attention(enhanced)
...
class Model8_RawNet_Wav2Vec2_FMSL(nn.Module):
    def __init__(self, d_args, device):
        super(Model8_RawNet_Wav2Vec2_FMSL, self).__init__()
        self.device = device
        ...
        # NOVEL: FMSL Layer for enhanced frequency-domain learning
        self.fmsl_layer = FMSLLayer(
            input_dim=d_args['filts'][0],
            num_filters=d_args.get('fmsl_filters', 64),
            kernel_size=d_args.get('fmsl_kernel_size', 7),
            dropout=d_args.get('fmsl_dropout', 0.1)
        )
    ...
    def forward(self, x):
        ...
        # NOVEL: Apply FMSL layer for enhanced frequency-domain features
        out = self.fmsl_layer(out)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Maze8: RawNet with Wav2Vec2 and FMSL (Novel Contribution)')
    ...
    parser.add_argument('--loss', type=str, default='focal', choices=['cce', 'focal'])
    parser.add_argument('--grad_clip_norm', type=float, default=1.0)
    parser.add_argument('--seed', type=int, default=1234)
    parser.add_argument('--model_path', type=str, default=None, help='Path to load a pre-trained model')
    parser.add_argument('--comment', type=str, default="Real_Maze_8", help='Comment for model directory')
    ...
    model_config = {
        'model': {
            ...
            # FMSL parameters (NOVEL CONTRIBUTION)
            'fmsl_filters': 64,
            'fmsl_kernel_size': 7,
            'fmsl_dropout': 0.1
        }
    ...
    # Initialize model
    model = Model8_RawNet_Wav2Vec2_FMSL(model_config['model'], device)
    if args.model_path:
        model.load_state_dict(torch.load(args.model_path, map_location=device))
    
    nb_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f'Trainable model parameters: {nb_params / 1e6:.2f} M')
    print(f'FMSL Layer Parameters: {sum(p.numel() for p in model.fmsl_layer.parameters() if p.requires_grad) / 1e3:.1f} K')
    ...
    # Loss function - Focal Loss recommended for FMSL
    if args.loss == 'focal':
    ...
    print("NOVEL CONTRIBUTION: FMSL Layer for enhanced frequency-domain learning")
    ...
    print(f'FMSL Layer successfully integrated and trained!')
    writer.close()
