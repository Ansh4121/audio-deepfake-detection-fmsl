...
\begin{itemize}
\item \texttt{comprehensive\_thesis\_analyser.py} -- Primary analysis script that generates all thesis figures and tables. This script performs comprehensive analysis across all Maze models (baseline and FMSL-enhanced variants), loads CM score files from evaluation results, calculates key metrics (EER, minDCF, accuracy), generates comparison tables, and produces visualizations including: \texttt{maze\_models\_comparison.png}, \texttt{fmsl\_standardization\_analysis.png}, \texttt{bottleneck\_analysis.png}, \texttt{trend\_visualizations.png}, \texttt{comprehensive\_histogram.png}, \texttt{thesis\_results\_table.csv}, and \texttt{thesis\_results\_table.tex}. The script handles model renumbering (removing Maze~4, renumbering subsequent models) and ensures consistent formatting for thesis inclusion.
\item \texttt{comprehensive\_evaluation.py} -- Full evaluation pipeline providing model loading, inference execution, and metric calculation across all models. Handles checkpoint loading, batch processing, and CM score file generation.
\item \texttt{Eval.py} -- Maze~5-focused evaluation script generating detailed analysis dashboards including ROC curves, Precision-Recall curves, score distributions, \texttt{comprehensive\_analysis\_dashboard.png}, and \texttt{confusion\_matrix\_analysis.png}. Provides statistical validation and performance metrics specific to the optimal architecture.
\item \texttt{Maze2\_Eval.py}, \texttt{Maze3\_Eval.py}, \texttt{Maze5\_eval.py}, \texttt{Maze6\_Eval.py}, \texttt{Maze7\_Eval.py}, \texttt{Maze8\_Eval.py} -- Per-model evaluation scripts that load model checkpoints, run inference on the ASVspoof 2019 LA evaluation set, and generate CM score files. These scripts follow a standardized protocol: load audio data, apply preprocessing (16~kHz sampling, silence trimming, length normalization to 64,600 samples), extract features (raw waveform, spectrograms, or Wav2Vec2 embeddings depending on the model), run inference, and output scores in the format required by ASVspoof evaluation tools. Model renumbering (Maze~4 removed, 5--8 $\to$ 5--7) is applied in the analysis scripts.
\item \texttt{score\_file\_processor.py} -- Utility script for processing CM score files and calculating evaluation metrics from the generated score files.
\end{itemize}

\textbf{Models (training and inference):}
\begin{itemize}
\item \texttt{Thesis/01\_Models/01\_Baseline\_Models/}: \texttt{main.py}, \texttt{maze2.py}, \texttt{maze3.py}, \texttt{maze4.py}, \texttt{maze5.py}, \texttt{maze6.py}, \texttt{maze7.py}, \texttt{maze8.py} -- Baseline model architectures implementing the progressive complexity sequence. Each script contains the complete model definition, training loop, and checkpoint saving functionality.
\item \texttt{Thesis/01\_Models/02\_FMSL\_Enhanced\_Models/}: \texttt{main\_fmsl\_standardized.py}, \texttt{maze2\_fmsl\_standardized.py}, \texttt{maze3\_fmsl\_standardized.py}, \texttt{maze4\_fmsl\_standardized.py}, \texttt{maze5\_fmsl\_standardized.py}, \texttt{maze6\_fmsl\_standardized.py}, \texttt{maze7\_fmsl\_standardized.py}, \texttt{maze8\_fmsl\_standardized.py} -- FMSL-enhanced, standardized versions of each baseline model. These scripts integrate the FMSL layer (Feature Matching Self-Supervised Learning) into the baseline architectures using the standardized configuration for fair comparison. All FMSL models use consistent parameters (embedding dimensions, FMSL prototypes, angular margin) as defined in \texttt{standardized\_maze\_config.py}.
\end{itemize}

\textbf{FMSL Implementation and Utilities:}
\begin{itemize}
\item \texttt{Thesis/06\_Utilities/fmsl\_advanced.py} -- Core FMSL (Feature Matching Self-Supervised Learning) implementation. Contains the \texttt{AdvancedFMSLSystem} class that implements:
  \begin{itemize}
  \item L2 normalization for hypersphere projection of features
  \item Angular margin learning with AM-Softmax loss (scale $s=32.0$, margin $m=0.45$)
  \item Prototype-based spoof class modeling with learnable prototypes ($n=3$)
  \item Optional latent space augmentation for improved generalization
  \end{itemize}
  This is the shared FMSL layer imported by all \texttt{maze*\_fmsl\_standardized.py} models.
\item \texttt{Thesis/06\_Utilities/fmsl\_standardized\_config.py} -- Standardized FMSL configuration providing consistent parameters across all models. Exports \texttt{get\_standardized\_model\_config()} used for fair comparison.
\item \texttt{Thesis/standardized\_maze\_config.py} -- Python standardization configuration for fair comparison (see~\ref{sec:standardization}). Defines the \texttt{STANDARDIZED\_CONFIG} dictionary used by all evaluation scripts and FMSL-enhanced models to ensure consistent comparison parameters.
\item \texttt{Thesis/06\_Utilities/data\_preprocessor.py}, \texttt{model\_trainer.py} -- Data preprocessing and training utilities for ASVspoof 2019 LA dataset.
\item \texttt{Thesis/analyze\_maze\_configurations.py}, \texttt{verify\_maze\_configurations.py} -- Configuration consistency verification scripts.
\end{itemize}

\textbf{Notebooks (reproducibility):}
\begin{itemize}
\item \texttt{Thesis/08\_Notebooks/Colab\_Evaluation\_Notebook.ipynb}, \texttt{Thesis/08\_Notebooks/Unified\_Evaluation\_Colab.ipynb}, \texttt{Thesis/08\_Notebooks/Complete\_Thesis.ipynb} -- Jupyter notebooks for running evaluation in Google Colab and reproducing results. These notebooks provide step-by-step execution of model evaluation, figure generation, and result analysis in a cloud environment.
\end{itemize}
...
To regenerate any figure: (1) clone the repository from \url{https://github.com/Ansh4121/audio-deepfake-detection-fmsl}; (2) install dependencies (\texttt{requirements.txt} in repository root or \texttt{Thesis/07\_Configuration\_Files/requirements.txt}); (3) run \texttt{Thesis/02\_Evaluation\_Scripts/comprehensive\_thesis\_analyser.py} with the evaluation results data (CM score files). The script will generate all thesis figures in \texttt{Thesis/02\_Evaluation\_Scripts/thesis\_analysis\_results/}; copy the PNG files to \texttt{WUT-Thesis/img/} for inclusion in the thesis. For detailed usage instructions, see \texttt{Thesis/02\_Evaluation\_Scripts/README.md}. Raw evaluation results (CM scores, metrics) are stored in timestamped directories following the naming convention \texttt{evaluation\_results-YYYYMMDDTHHMMSSZ-*} or can be generated by running the per-model evaluation scripts (\texttt{Maze*\_Eval.py}).
